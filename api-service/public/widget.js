 ...
 * Легкий віджет для вбудовування на будь-який сайт
 */
...
(function() {
    ...
    const WIDGET_CONFIG = {
        apiUrl: 'http://localhost:5050', // URL API сервісу
        position: 'bottom-right', // Позиція: bottom-right, bottom-left, top-right, top-left
        theme: 'default', // Тема: default, dark, light
        language: 'uk', // Мова: uk, en
        autoInit: true // Автоматична ініціалізація
    };
    ...
    const WIDGET_HTML = `
        <div class="ai-widget" id="aiWidget">
            <div class="ai-widget-header">
                <div>
                    <h3 class="ai-widget-title">AI Swagger Bot</h3>
                    <p class="ai-widget-subtitle">Запитайте про API</p>
                </div>
    ...
    class AIWidget {
        ...
        createWidget() {
            ...
            this.widget = div.firstElementChild;
        ...
        setPosition() {
            ...
            Object.assign(this.widget.style, position);
        }
        ...
        toggleWidget() {
            ...
            this.widget.classList.toggle('ai-widget-minimized', this.isMinimized);
        ...
        async sendMessage() {
            ...
            try {
                const response = await fetch(`${this.config.apiUrl}/api/chat`, {
                    ...
                    body: JSON.stringify({
                        message: message,
                        api_spec_url: this.getApiSpecUrl()
                    })
                });
                ...
                this.hideTyping();
                this.addMessage('Вибачте, сталася помилка. Спробуйте ще раз.', 'bot');
            }
        ...
        getApiSpecUrl() {
            return window.location.origin + '/api/swagger.json';
        }
        ...
        // Публічні методи для керування віджетом
        show() {
    ...
    // Глобальний об'єкт для доступу до віджету
    window.AIWidget = AIWidget;

    // Автоматична ініціалізація
    if (WIDGET_CONFIG.autoInit) {
        document.addEventListener('DOMContentLoaded', () => {
            window.aiWidgetInstance = new AIWidget();
        });
    }
