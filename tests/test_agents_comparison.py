...
def test_interactive_agent():
    ...
    try:
        ...
        if response.get('needs_followup'):
            ...
            followup_response = agent.process_followup_query(followup_query, user_identifier="test_user")
        ...
        for i, interaction in enumerate(history[-3:], 1):
            ...
            print(f"   Бот [{interaction.get('status', 'unknown')}]: {interaction.get('bot_response', '')[:100]}...")
