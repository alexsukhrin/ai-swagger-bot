...
def test_admin_filters():
    ...
    try:
        response = requests.get(f"{base_url}/admin/prompttemplate/list")
        if response.status_code == 200:
    ...
    try:
        response = requests.get(f"{base_url}/admin/swaggerspec/list")
        if response.status_code == 200:
    ...
    try:
        response = requests.get(f"{base_url}/admin/chatsession/list")
        if response.status_code == 200:
    ...
    try:
        response = requests.get(f"{base_url}/admin-db/apicall/list")
        if response.status_code == 200:
