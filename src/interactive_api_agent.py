...
class InteractiveSwaggerAgent:
    ...
    def __init__(self, swagger_spec_path: str, enable_api_calls: bool = False, 
                 openai_api_key: Optional[str] = None, jwt_token: Optional[str] = None):
        """
        ...
        try:
            ...
            if not self.openai_api_key:
                raise ValueError("OPENAI_API_KEY не знайдено. Додайте в .env файл або передайте як параметр.")
    ...
    def process_interactive_query(self, user_query: str, user_identifier: str = "default_user") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not intent:
                ...
                self.conversation_history.add_interaction(user_id, {
    ...
    def process_followup_query(self, user_query: str, user_identifier: str = "default_user") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Оновлюємо API запит
            updated_api_request = self._update_api_request_with_followup(api_request, updated_intent)
    ...
    def _analyze_error_and_generate_followup(self, api_response: Dict[str, Any], 
        ...
        try:
            ...
            elif 'unauthorized' in error_message.lower() or '401' in str(api_response.get('status_code')):
                return "🔐 Помилка авторизації. Перевірте ваш JWT токен."
            elif 'not found' in error_message.lower() or '404' in str(api_response.get('status_code')):
                return "🔍 Ресурс не знайдено. Перевірте правильність ID або шляху."
            elif 'conflict' in error_message.lower() or '409' in str(api_response.get('status_code')):
    ...
    def _generate_validation_followup(self, api_request: Dict[str, Any], 
                                    intent: Dict[str, Any], 
...
💡 Будь ласка, уточніть ваш запит або надайте додаткову інформацію.
    ...
    def _update_intent_with_followup(self, original_intent: Dict[str, Any], 
                                   followup_query: str) -> Dict[str, Any]:
    ...
    def _update_api_request_with_followup(self, original_request: Dict[str, Any], 
                                        updated_intent: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _form_api_request(self, user_query: str, intent: Dict[str, Any], 
                         endpoints: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _find_best_endpoint(self, intent: Dict[str, Any], 
                           endpoints: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _get_endpoint_details(self, endpoint: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for ep in self.parser.get_endpoints():
            if (ep.method == metadata.get('method') and 
                ep.path == metadata.get('path')):
                return {
                    'method': ep.method,
                    'path': ep.path,
                    'summary': ep.summary,
    ...
    def _validate_and_form_request(self, intent: Dict[str, Any], 
                                 endpoint_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
            ...
            for param in endpoint_info.get('parameters', []):
                if hasattr(param, 'location') and param.location == 'path':
                    ...
                    if param_name in intent_params:
                        # Замінюємо в шляху
                        path = endpoint_info['path'].replace(f"{{{param_name}}}", str(intent_params[param_name]))
    ...
    def _format_response(self, api_request: Dict[str, Any], 
    ...
    def _process_api_response_with_gpt(self, api_request: Dict[str, Any], 
...
"""
    ...
    def _form_creation_request(self, endpoint_info: Dict[str, Any], object_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _handle_creation_error(self, response: Dict[str, Any], creation_response: str, user_query: str) -> str:
...
"""
    ...
    def _is_creation_request(self, user_query: str) -> bool:
        ...
        creation_keywords = [
            "створи", "create", "додай", "add", "новий", "new",
            "категорію", "category", "товар", "product", "користувача", "user"
        ]
...
"""
    ...
    def _format_basic_response(self, api_request: Dict[str, Any], 
                             api_response: Dict[str, Any]) -> str:
