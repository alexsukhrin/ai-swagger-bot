...
class EnhancedSwaggerParser:
    ...
    def _parse_schema_field(self, name: str, field_data: Dict[str, Any], required: bool) -> SchemaField:
        """Парсить поле схеми."""
        ...
        for endpoint in endpoints:
            ...
            chunk = {
                ...
                'metadata': {
                    ...
                    'required_parameters': ', '.join(endpoint.required_parameters) if endpoint.required_parameters else '',
                    'optional_parameters': ', '.join(endpoint.optional_parameters) if endpoint.optional_parameters else '',
                    'path_variables': ', '.join(endpoint.path_variables) if endpoint.path_variables else '',
                    'query_parameters': ', '.join(endpoint.query_parameters) if endpoint.query_parameters else ''
                }
            }
            chunks.append(chunk)
                            ...
                            if schema_info.properties:
                                ...
                                for field_name, field in schema_info.properties.items():
                                    ...
                                    if field.enum_values:
                                        field_desc += f" [Enum: {', '.join(map(str, field.enum_values))}]"
                                    if field.example:
